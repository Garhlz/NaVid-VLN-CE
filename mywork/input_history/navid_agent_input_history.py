...
import random
from collections import deque # <-- 核心改动：导入 deque 来实现高效的滑动窗口
...
class NaVid_Agent(Agent):
    def __init__(self, model_path, result_path, require_map=True):
        ...
        print("Initialize NaVid Agent with Sliding Window Prompting")
        ...
        # 1. 
        # =============================================================
        # Prompt 模板与“保守策略”完全一致，因为这个结构被证明是模型最能理解的。
        self.promt_template = (
            "Imagine you are a robot programmed for navigation tasks. "
            "You have been given a video of historical observations and an image of the current observation <image>. "
            "Your assigned task is: '{instruction}'. "
            "For your reference, your own recent actions in reverse order were: [{action_history}]. " # 占位符不变
        ...
        # 2.
        # =============================================================
        # 使用 deque (双端队列) 并设置 maxlen 来创建一个固定大小的“滑动窗口”。
        # 当我们向满的 deque 中添加新动作时，最旧的那个动作会自动被丢弃。
        # 这完美地解决了内存和 Token 无限增长的问题。
        # maxlen=10 是一个可以调整的超参数，代表模型的短期记忆容量。
    ...
    def act(self, observations, info, episode_id):
        ...
        if len(self.pending_action_list) != 0 :
            ...
            # 无论何时执行动作，都将其添加到滑动窗口中
            self.action_history.append(action_to_perform)
            
            if self.require_map:
                img = self.addtext(output_im, observations["instruction"]["text"], "Pending action: {}".format(action_to_perform))
        ...
        # 准备历史动作字符串, self.action_history 是一个固定大小的 deque
        if not self.action_history:
            history_str = "None"
        else:
            action_map = {0: "STOP", 1: "MOVE_FORWARD", 2: "TURN_LEFT", 3: "TURN_RIGHT"}
            # deque 可以像 list 一样被迭代和反转
            action_names = [action_map.get(a, "UNKNOWN") for a in self.action_history]
            history_str = ", ".join(reversed(action_names))

        # 使用指令文本和固定长度的历史动作字符串来格式化 Prompt
        instruction_text = observations["instruction"]["text"]
        navigation_qs = self.promt_template.format(instruction=instruction_text, action_history=history_str)
        ...
        if action_index is None or len(self.pending_action_list) == 0:
            action_to_perform = random.randint(1, 3)
            self.pending_action_list.append(action_to_perform)
